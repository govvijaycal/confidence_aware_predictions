...
class RoadEntry:
	...
	def visualize(self):
		print( "Initial yaw: {}".format(self.yaws[0]) )
		plt.plot(self.left_side[:,0], self.left_side[:,1], 'r')
		plt.plot(self.centerline[:,0], self.centerline[:,1], 'k')
		plt.plot(self.right_side[:,0], self.right_side[:,1], 'b')

		plt.scatter(self.x_min, self.y_min, marker='x', color='g')
		plt.scatter(self.x_max, self.y_max, marker='o', color='g')
		plt.show()

class SemanticRasterizer:
	def __init__(self,
				 road_entries,           # List of road entries.
		...
		self.road_entries = road_entries
		self.bounds = np.array([[re.x_min, re.y_min, re.x_max, re.y_max] for re in self.road_entries])
		...
		self.ego_to_pixel = np.array([[1./raster_resolution,  0., ego_center_x],\
									  ...
									  [0., 0., 1.]])
		#self.raster_radius = np.linalg.norm(raster_resolution * np.array([*raster_size])) / 2. # TODO
	...
	def rasterize(self, agent_history):		
		...
		render_xmin, render_ymin, _ = pixel_to_world @ np.array([0., 0., 1.])
		render_xmax, render_ymax, _ = pixel_to_world @ np.array([self.raster_width, self.raster_height, 1.])

		render_bounds = np.array([render_xmin, render_ymin, render_xmax, render_ymax])
		...
		# TODOS: see if further speedup can be done (without caching for now).  Add traffic light rendering.
