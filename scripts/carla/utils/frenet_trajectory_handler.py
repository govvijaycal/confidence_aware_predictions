...
class FrenetTrajectoryHandler(object):
    ...
    def get_left_boundary(self, offset):
        xy_left = []
        ...
        for state in self.trajectory:
            ...
            left_yaw = fwd_yaw + np.pi/2.

            x_l = x + offset * np.cos( left_yaw )
            y_l = y + offset * np.sin( left_yaw )
            xy_left.append([x_l, y_l])

        return np.array(xy_left)

    def get_right_boundary(self, offset):
        return self.get_left_boundary(offset=-offset)
